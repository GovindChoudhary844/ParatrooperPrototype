...
public class EnemyMovement : MonoBehaviour
{
    ...
    [Header("Enemy(s)")]
    [SerializeField] private List<Transform> enemy;
    public List<Transform> EnemyList => enemy;
        ...
        while (targetIndex < stairPoints.Count)
        {
            ...
            if (Vector3.Distance(currentPos, targetPos) <= 0.01f)
            {
                    ...
                    else
                    {
                        switch (currentWayPointIndex)
                        {
                            case 1: // vertical step
                                targetPos = new Vector3(currentPos.x,
                                                        stairPoints[2].position.y,
                                                        currentPos.z);
                                break;
                            ...
                            case 2: // horizontal step
                                {
                                    ...
                                    float offsetX = (targetIndex == 3)
                                                    ? stairPoints[targetIndex].localScale.x
                                                    : 0f;
                                    targetPos = new Vector3(basePos.x - offsetX * moveDirection,
                                                            currentPos.y,
                                                            currentPos.z);
                                }
                                break;
                            ...
                            case 3: // vertical step
                                targetPos = new Vector3(currentPos.x,
                                                        stairPoints[targetIndex].position.y,
                                                        currentPos.z);
                                break;
                ...
                else
                {
                    ...
                    targetPos = stairPoints[targetIndex].position;
                    enemyTf = enemy[targetIndex];
                    currentPos = enemyTf.position;

                    if (targetIndex > 1)
                    {
                        currentMaxWayPointCount = (targetIndex % 2 == 0) ? targetIndex : targetIndex + 1;
                        targetPos = stairPoints[1].position
                                    - Vector3.right * stairPoints[1].localScale.x * moveDirection;
                    }
                }
            }
            yield return null;
